    ...
    var core = {
      ...
      init: function init() {
        var defaults = {
          ...
          elementClass: 'faq', // element class {string}
          triggerClass: 'faq-trigger', // trigger class {string}
          panelClass: 'faq-panel', // panel class {string}
      ...
      generateIDs: function generateIDs(element) {
        ...
        element.setAttribute('id', element.id || 'faq-'.concat(uniqueId));
        trigger.setAttribute('id', trigger.id || 'faq-trigger-'.concat(uniqueId));
        panel.setAttribute('id', panel.id || 'faq-panel-'.concat(uniqueId));
      },
      ...
      removeIDs: function removeIDs(element) {
        ...
        if (element.id.startsWith('faq-')) element.removeAttribute('id');
        if (trigger.id.startsWith('faq-')) trigger.removeAttribute('id');
        if (panel.id.startsWith('faq-')) panel.removeAttribute('id');
      },
...
const arrowdown = `width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M15 7.5L10 12.5L5 7.5" stroke="#FAFAFA" stroke-width="2"/>`
    if (textShown === false) {
        faqSVG.innerHTML = arrowup;
        textShown = true;
    } else {
        faqSVG.innerHTML = arrowdown;
        textShown = false;
    }
}
